...
def main() -> None:
    ...
    pool_path = paths.processed_dir / "train_windows_norm_pool_v2.npz"
    if not pool_path.exists():
        raise FileNotFoundError(f"Missing normalized pool v2 file: {pool_path}")

    models_dir = Path("models")
    models_dir.mkdir(parents=True, exist_ok=True)

    out_model = models_dir / "pca_pool_v2.joblib"
    out_scores = paths.processed_dir / "pca_pool_v2_train_scores.csv"
    out_thresholds = paths.processed_dir / "pca_pool_v2_thresholds.json"

    data = np.load(pool_path, allow_pickle=True)
    Xw = data["X"]  # (n_windows, window_size, n_features)

    if np.isnan(Xw).any() or np.isinf(Xw).any():
        raise ValueError("NaN/Inf detected in normalized pool v2")

    n_windows, window_size, n_features = Xw.shape
    X = Xw.reshape(n_windows, window_size * n_features).astype(np.float64)

    print(f"[INFO] Loaded normalized pool v2: {pool_path}")
    ...
    thresholds = {
        "method": "PCA reconstruction MSE (trained on normalized strict-normal pool v2)",
